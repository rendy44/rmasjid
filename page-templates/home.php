...
while ( have_posts() ) {
			...
			<div class="carousel-inner">
				...
				foreach ( $sliders as $slider ) {
                    ...
                    <div class="carousel-item <?php echo $active_slider; // phpcs:ignore ?>">
						<?php
						$background_inline = ! empty( $slider['bg'] ) ? 'style="' . Helpers\Helper::get_header_style_inline( $slider['bg'] ) . '"' : '';
						?>
                        <div class="carousel-item-wrapper d-flex" <?php echo $background_inline; // phpcs:ignore ?>>
							<div class="container">
								<?php
	...
	<!-- Shalat times -->
	<?php
	$salat         = (array) get_option( 'salat_times' );
	$salat_date    = get_option( 'salat_times_date' );
	$salat_city    = get_option( 'salat_times_city' );
	$salat_country = get_option( 'salat_times_country' );
	$now_date      = date( 'Y-n-j' );
	if ( ( $now_date === $salat_date ) && ! empty( $salat ) ) {
		?>
		<section id="salat">
			<div class="bg-sky fajr">
				<div class="sun"></div>
				<div class="stars">
					<span class="light"></span> <span></span> <span class="bright"></span> <span></span> <span class="light"></span> <span class="light"></span> <span class="bright"></span> <span></span> <span class="bright"></span> <span></span>
				</div>
				<div class="container">
					<div class="row">
						<div class="col-lg-8 mx-auto text-center">
                            <p class="lead mb-4"><?php echo __( 'Today prayer times in', 'masjid' ) . ' ' . $salat_city . ', ' . $salat_country; // phpcs:ignore ?></p>
							<ul class="box-info">
								<li>
									<span><?php echo __( 'Fajr', 'masjid' ); ?></span> <strong><?php echo $salat['fajr']; // phpcs:ignore ?></strong>
								</li>
								<li>
									<span><?php echo __( 'Shurooq', 'masjid' ); ?></span> <strong><?php echo $salat['shurooq']; // phpcs:ignore ?></strong>
								</li>
								<li>
									<span><?php echo __( 'Dhuhr', 'masjid' ); ?></span> <strong><?php echo $salat['dhuhr']; // phpcs:ignore ?></strong>
								</li>
								<li>
									<span><?php echo __( 'Ashr', 'masjid' ); ?></span> <strong><?php echo $salat['asr']; // phpcs:ignore  ?></strong>
								</li>
								<li>
									<span><?php echo __( 'Maghrib', 'masjid' ); ?></span> <strong><?php echo $salat['maghrib']; // phpcs:ignore ?></strong>
								</li>
								<li>
									<span><?php echo __( 'Isha', 'masjid' ); ?></span> <strong><?php echo $salat['isha']; // phpcs:ignore ?></strong>
								</li>
							</ul>
						</div>
					</div>
